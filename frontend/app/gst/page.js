...
const GSTForm = ({ onSubmit }) => {
    ...
    useEffect(() => {
        const fetchHotelAndGSTPercentage = async () => {
            try {
                const hotelsResponse = await axios.get('http://ec2-15-206-189-233.ap-south-1.compute.amazonaws.com:5000/api/hotel/get-all');
    ...
    useEffect(() => {
        const fetchGSTPercentage = async () => {
            try {
                if (hotel._id) {
                    const gstResponse = await axios.get(`http://ec2-15-206-189-233.ap-south-1.compute.amazonaws.com:5000/api/hotel/get/${hotel._id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        if (hotel._id && gstPercentage !== '') {
            try {
                await axios.patch(`http://ec2-15-206-189-233.ap-south-1.compute.amazonaws.com:5000/api/hotel/gst/${hotel._id}`, {
