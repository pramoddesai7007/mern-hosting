    ...
    useEffect(() => {
        const fetchACPercentage = async () => {
            try {
                ...
                const tableResponse = await axios.get(`http://ec2-15-206-189-233.ap-south-1.compute.amazonaws.com:5000/api/table/tables/${tableId}`);
                const table = tableResponse.data;

                // Fetch the section information using the section ID
                const sectionResponse = await axios.get(`http://ec2-15-206-189-233.ap-south-1.compute.amazonaws.com:5000/api/section/${table.section._id}`);
