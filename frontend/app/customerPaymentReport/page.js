...
const CustomerPaymentReport = () => {
  ...
  const fetchDatewiseRecords = async () => {
    try {
      const response = await axios.get(
        `http://ec2-15-206-189-233.ap-south-1.compute.amazonaws.com:5000/api/customer/datewiseRecords?startDate=${startDate}&endDate=${endDate}`
      );
  ...
  const fetchCustomers = async () => {
    try {
      const response = await axios.get(
        "http://ec2-15-206-189-233.ap-south-1.compute.amazonaws.com:5000/api/customer/customers"
      );
      console.log(response.data);
      setCustomers(response.data);
    } catch (error) {
